...
(defun scilab-ltype-function-comm ()
  "Return t if the current line is a comment and the previous line 
contains function."
  (save-excursion
    (if (not (scilab-ltype-comm))
        nil
...
(defun scilab-calc-indent ()
  ...
  (if (save-excursion 
      ...
      (let* ((ci (save-excursion (or (scilab-prev-line)
                                     (progn (beginning-of-line)
                                            (forward-line -1)))
                                 (scilab-beginning-of-command)
                                 (scilab-next-line-indentation))))
        (- ci scilab-indent-level))
...
Do it only once. Opposite action to the `scilab-comment-region'. END-REGION are arguments which specify the region boundaries."
    ...
    (let ((end-region-mark (make-marker)) (save-point (point-marker)))
      ...
      (set-marker end-region-mark nil)
      (set-marker save-point nil))
    (indent-region beg-region end-region nil)
    )
