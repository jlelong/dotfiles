...
"	04nov2011   jlelong				Add very basic syntax based folding
"
...
" " Reserved words.
syn keyword scilabStatement abort clear clearglobal exit global mode predef quit resume
syn keyword scilabStatement 		 return
syn keyword scilabFunction contained function endfunction funptr 
...
syn keyword scilabRepeat contained for while break
syn keyword scilabConditional contained if then else elseif
syn keyword scilabMultiplex contained select case
syn keyword scilabEnd contained end
...
"Fold
syn match scilabCommand              "\<\h\w*\>"       contains=scilabEnd,scilabRepeat,scilabConditional,scilabMultiplex
syn region scilabComments start="^\zs\s*//.*\_s*//" skip="^\s*//" end='^\ze\s*\([^/][^/]\|$\)' fold contains=scilabComment 
syn region scilabFunctionFold start="^function.*$" end="^endfunction\s*$" keepend contains=ALL transparent fold
syn region scilabBlockFold start=+^[^/"]*[^/"]*\<\(if\|for\|select\|while\)\>+ end="\<end\>" skip="^\s*//" fold containedin=ALLBUT,scilabComments,scilabString keepend extend  transparent 
...
if version >= 508 || !exists("did_scilab_syntax_inits")
	...
	HiLink	scilabIdentifier			Normal
