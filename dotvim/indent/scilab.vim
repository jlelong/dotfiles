...
setlocal indentexpr=GetScilabIndent()
setlocal indentkeys=!,o,O=end,=case,=else,=elseif,=otherwise,=catch,=endfunction
...
function! s:IsScilabContinuationLine(lnum)
...
function GetScilabIndent()
  ...
  if s:IsScilabContinuationLine(v:lnum - 1) 
  ...
  if getline(plnum) =~ '^\s*\(classdef\|properties\|switch\|methods\|events\|function\|if\|while\|for\|otherwise\|case\|try\|catch\|else\|elseif\)\>'
    let curind = curind + &sw
    " In Scilab we have different kind of functions
    ...
    "   'indent all functions' in the Scilab Editor)
  ...
  if getline(v:lnum) =~ '^\s*\(else\|elseif\|end\|catch\|otherwise\|case\|endfunction\)\>'
    let curind = curind - &sw
  endif
