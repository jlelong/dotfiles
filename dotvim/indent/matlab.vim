...
function GetMatlabIndent()
  ...
  if getline(plnum) =~ '^\s*\(classdef\|properties\|switch\|methods\|events\|function\|if\|while\|for\|otherwise\|case\|try\|catch\|else\|elseif\|do\)\>'
    let curind = curind + &sw
  ...
  if getline(v:lnum) =~ '^\s*\(else\|elseif\|end\|catch\|otherwise\|case\|until\)\>'
    let curind = curind - &sw
  endif
